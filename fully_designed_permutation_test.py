...
# ----------------------------------------------------------------------
B_REPLICATES = 100
INNER_B = 500      # refit-per-permutation is much cheaper than the old
...
gamma2_values = np.array([2.0, 2.05, 2.1, 2.15, 2.2, 2.25, 2.3, 2.35, 2.4, 2.45, 2.5, 2.55, 2.6, 2.65, 2.7, 2.75, 2.8, 2.85, 2.9, 2.95, 3.0])
...
#
# KNN / SVM / GCN now use refit-per-permutation on the FULL sample
# (train == test == all N_SAMPLES*2 graphs), matching the paradigm
# Ginestet/Dubey/Lovato already use. See permutation_test_refit.
# ----------------------------------------------------------------------
def run_one_replicate(gamma2, rep):
    ...
    base_gcn_data = gcn.build_base_data(G_all)
    ...
    _, p_val, _ = permutation_test_refit(
        lambda y_labels: gcn.fit_predict_full(base_gcn_data, y_labels,
